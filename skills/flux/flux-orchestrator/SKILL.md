...
| Delegar `vision_analyze` via `delegate_task` | Sub-agentes herdam o modelo do orquestrador; modelos sem visão (GLM-5.1, deepseek-v4-pro) falham, modelos de visão (qwen3-vl) dão timeout em 600s, e o bug de `reasoning_details` no kimi-k2.6 (opencode-go) quebra pipelines multi-turn | **NUNCA delegar `vision_analyze` via `delegate_task`.** Workaround: pedir ao usuário para enviar imagens diretamente no chat (fotos no Telegram). Máximo 1 tentativa; se falhar, usar copy inferida com disclaimer: "⚠️ Copy gerada sem análise visual — validar com referências do cliente." |
...
| Sub-agentes usam o modelo errado | `delegate_task` não suporta override de modelo por task — sub-agentes herdam o modelo do Orquestrador. Tarefas criativas (copy, design, prompt) rodam com modelo analítico | Workaround: Orquestrador troca para GLM-5.1 (melhor para prompt engineering) antes de delegar tarefas criativas, e volta para deepseek-v4-pro apenas na síntese final. Documentado em `references/carousel-workflow-example.md`. |
