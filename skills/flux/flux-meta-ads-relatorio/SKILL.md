...
1. **Contexto do cliente:** Leia `.agents/contexts/[cliente]/product-marketing-context.md` — nome de exibição, unidades, diferenciais
2. **Ferramentas:** Meta Ads MCP + script `scripts/gerar-relatorio.py`. Token em `/opt/data/.env`
3. **Regras de marca:** Consulte `shared/glossario.md` — em relatório para cliente: "investimento" não "spend", "potencial cliente" não "lead"
4. **Pitfalls:** Leia a seção abaixo ANTES de qualquer chamada de API
